...
4. **大模型评测（Eval Harness）缺乏基础设施**：业务团队在对比不同模型（如 Gemini 1.5 Pro vs GPT-4o）的响应质量、延迟和单次成本时缺乏统一的基准测试中间件。
...
* 提供 **FastAPI 业务中间件与 Benchmark 测试接口**，方便自动化跑分与成本监控。
...
3. **Data & Storage Layer**
   * **PostgreSQL (15+)**：持久化存储请求日志（`request_logs`）、模型单价配置（`model_pricing`）及评测结果（`eval_benchmarks`）。
...
* **API 兼容性**：网关必须 100% 兼容 `POST /v1/chat/completions` 标准。
* **支持模型列表**：
  * `openai/gpt-4o` / `openai/gpt-4o-mini`
  * `gemini/gemini-1.5-pro` / `gemini/gemini-1.5-flash` (via GCP Vertex AI / Service Account)
  * `anthropic/claude-3-5-sonnet`
* **Fallback 容灾规则**：
  * 当主模型返回 `429 (Rate Limit)`、`500` 或 `503` 时，自动在 500ms 内重试切换至备用模型（例如 `gpt-4o` 报错时自动切至 `gemini-1.5-pro`）。

### 3.2 费用审计与 Token 计量 (Cost Audit & Token Metering)
* **精准 Token 统计**：必须提取每次调用的 `prompt_tokens`、`completion_tokens` 和 `total_tokens`。
* **美元花费计算 (USD Cost)**：网关须根据模型单价（如 Input $2.50/M, Output $10.00/M）实时计算并记录本次调用的成本（精确到小数点后 6 位）。
* **PostgreSQL 日志落库**：每次调用必须异步写入 PostgreSQL 的 `llm_request_logs` 表，严禁阻塞主调用链。

### 3.3 缓存与速率限制 (Caching & Rate-Limiting)
* **Redis 精确匹配缓存**：对于相同的 `messages` 输入，支持在 Redis 中配置 TTL（如 3600s）直接返回缓存结果，降低 API 开销。
* **滑动窗口限流**：支持按 API Key 限制每分钟请求数（RPM）和每分钟 Token 数（TPM）。

### 3.4 FastAPI 中间件与 Eval Harness 接口
* **`POST /v1/eval/run`**：支持传入一组 Prompt 测试集，并发向多个指定的模型发送请求，收集并返回各模型的响应延迟（TTFT/Latency）、Token 消费及质量评估得分。
* **`GET /v1/metrics/spend`**：支持查询按天、按模型或按 API Key 统计的累计美元开销。

---

## 4. 数据库 Schema 设计 (Database Schema)

在 PostgreSQL 中创建以下核心数据表结构：
...
POSTGRES_PORT=5432
DATABASE_URL=postgresql://litellm_user:litellm_secure_password@127.0.0.1:5432/litellm_db
...
LITELLM_MASTER_KEY=sk-master-key-for-admin
LITELLM_PORT=4000
FASTAPI_PORT=8000
```
...
为了帮助主人高效完成项目的搭建与练手，划分为以下四个步骤逐一推进：

### 阶段一：本地基础设施与 LiteLLM 代理启动 (Phase 1)
- [ ] 安装依赖并启动本地 PostgreSQL 与 Redis 服务（通过 Docker 或 Apt）。
...
### 3. 阶段三：FastAPI 中间件与 Eval Harness 开发 (Phase 3)
- [ ] 编写 FastAPI 主程序 `main.py`，暴露业务接口。
- [ ] 实现 `/v1/eval/run` 评测接口：使用 Python 协程并发测试多个模型的响应速度与开销。
...
- [ ] 编写 Systemd 服务文件 `litellm.service` 与 `fastapi.service`，实现开机自启、掉线重启与双进程守护。
- [ ] 进行完整联调与压力测试。

---

*文档完结 - 随时准备与主人一起动手写代码实施！*
